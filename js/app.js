...
(function () {
  ...
  var HomeMap = {
    ...
    buildMarkers: function () {
      ...
      list.forEach(function (lm, i) {
        ...
        /* 已通关（满分）的地标 → 学习之星徽章 */
        if (window.Quiz && window.Quiz.isCleared && window.Quiz.isCleared(lm.id)) {
          floatLayer.appendChild(el('div', 'hm-star', '★'));
        }
  ...
  function buildLandmarkCard() {
    ...
    p.innerHTML =
      ...
      '<div class="lm-card">' +
        ...
        '<div class="lm-gallery" id="lmGallery">' +
          ...
          '<div class="lg-vbadge" id="lgVbadge"><span class="lgv-play">▶</span><span class="lgv-txt">舞蹈短片</span><span class="lgv-dur" id="lgVdur"></span></div>' +
        ...
        '<div class="lm-body">' +
          ...
          /* 特色舞蹈视频：常驻展示、位置靠前，强力提升视频权重 */
          '<div class="lm-feature" id="lmFeature">' +
            '<button class="lf-poster" id="lfPoster" type="button" aria-label="播放舞蹈短片">' +
              '<span class="lf-play">▶</span>' +
              '<span class="lf-flag">原创舞蹈影像</span>' +
              '<span class="lf-zoom">⤢ 点击全屏观看</span>' +
              '<span class="lf-dur" id="lfDur"></span>' +
            '</button>' +
          '</div>' +
          '<div class="lm-tabs">' +
          ...
          '<div class="lm-video" id="lmVideo">' +
            '<div class="video-box">' +
              ...
              '<button class="video-fs" id="videoFs" type="button" aria-label="全屏放大">⤢</button>' +
              '<div class="video-cover" id="videoCover">' +
                '<span class="vc-play">▶</span>' +
                '<span class="vc-tip">点击播放舞蹈短片</span>' +
                '<span class="vc-dur" id="vcDur"></span>' +
              '</div>' +
        ...
        '</div>' +
      '</div>' +
      /* 视频全屏灯箱：点击放大、切换铺满 / 完整画面、支持双指缩放 */
      '<div class="video-lightbox" id="videoLightbox">' +
        '<div class="vl-mask" data-close></div>' +
        '<div class="vl-stage" id="vlStage">' +
          '<video id="vlVideo" playsinline webkit-playsinline controls preload="none"></video>' +
          '<button class="vl-zoom" id="vlZoom" type="button" aria-label="切换铺满">⤢</button>' +
          '<button class="vl-close" id="vlClose" type="button" aria-label="关闭">×</button>' +
          '<div class="vl-tip" id="vlTip">双指可缩放 · 点 ⤢ 切换铺满 / 完整画面</div>' +
        '</div>' +
      '</div>';
    ...
    /* 视频全屏灯箱：特色视频条 / 头图角标 / 视频页放大按钮 共用 */
    var lb = $('#videoLightbox', p);
    if (lb) {
      $$('[data-close]', lb).forEach(function (n) { n.addEventListener('click', closeVideoLightbox); });
      var vlc = $('#vlClose', lb);
      if (vlc) vlc.addEventListener('click', function (e) { e.stopPropagation(); closeVideoLightbox(); });
      var vz = $('#vlZoom', lb);
      if (vz) vz.addEventListener('click', function (e) {
        e.stopPropagation();
        $('#vlStage', lb).classList.toggle('is-cover');
      });
    }
    var fsBtn = $('#videoFs', p);
    if (fsBtn) fsBtn.addEventListener('click', function (e) {
      e.stopPropagation(); openVideoLightbox();
    });
    var feat = $('#lmFeature', p);
    if (feat) feat.addEventListener('click', function () { openVideoLightbox(); });
    var vbadge = $('#lgVbadge', p);
    if (vbadge) vbadge.addEventListener('click', function (e) {
      e.stopPropagation(); openVideoLightbox();
    });
  }
  ...
  /* ---------- 视频全屏灯箱（放大观看，避免手机端放大发虚） ---------- */
  function openVideoLightbox() {
    var p = $('#lmPanel');
    if (!p) return;
    var lm = currentLandmark;
    if (!lm || !lm.video) return;
    /* 暂停卡片内视频，避免双重声音 */
    stopVideo();
    var box  = $('#videoLightbox', p);
    var v    = $('#vlVideo', p);
    var stage = $('#vlStage', p);
    if (!box || !v) return;
    stage.classList.remove('is-cover');        // 默认完整画面（contain）
    v.muted = false;                            // 主动全屏观看 → 带声
    v.src = lm.video;
    if (lm.poster) v.poster = lm.poster;
    v.controls = true;
    box.classList.add('is-on');
    document.body.style.overflow = 'hidden';
    var pr = v.play();
    if (pr && pr.catch) pr.catch(function () { /* 等待用户手动点 */ });
  }
  ...
  function openLandmark(lm) {
    ...
    /* 封面帧：未播放时显示视频海报，播放中由 video 自身接管 */
    if (lm.poster) {
      v.poster = lm.poster;
      cover.style.backgroundImage = 'url(' + lm.poster + ')';
    } else {
      cover.style.backgroundImage = '';
    }
    var durEl = $('#vcDur', p);
    if (durEl) durEl.textContent = lm.videoDuration || '';

    /* 特色视频条 + 头图角标：把视频前置曝光，提升权重 */
    var fp = $('#lfPoster', p);
    if (fp) fp.style.backgroundImage = lm.poster ? 'url(' + lm.poster + ')' : '';
    var fd = $('#lfDur', p);
    if (fd) fd.textContent = lm.videoDuration || '';
    var vb = $('#lgVbadge', p);
    if (vb) vb.style.display = lm.video ? '' : 'none';
    var featEl = $('#lmFeature', p);
    if (featEl) featEl.style.display = lm.video ? '' : 'none';
    var vd = $('#lgVdur', p);
    if (vd) vd.textContent = lm.videoDuration || '';
    var vlV = $('#vlVideo', p);
    if (vlV && lm.poster) vlV.poster = lm.poster;

    /* 视频下方：重庆文旅官方矩阵快捷入口 */
    var vl = $('#videoLinks', p);
    if (vl) fillSheetLinks(vl, CFG.site.social && CFG.site.social.links);
  ...
  /* ---------- 学习之星（关卡满分点亮，首页 marker 显示） ---------- */
  function refreshHomeStars() {
    $$('#homeMap .hm-marker').forEach(function (m) {
      var id = m.dataset.id;
      var has = m.querySelector('.hm-star');
      var need = window.Quiz && window.Quiz.isCleared && window.Quiz.isCleared(id);
      if (need && !has) {
        var fl = m.querySelector('.hm-mk-float');
        if (fl) fl.appendChild(el('div', 'hm-star', '★'));
      } else if (!need && has) {
        has.remove();
      }
    });
  }
  window.addEventListener('cq:clear', refreshHomeStars);
