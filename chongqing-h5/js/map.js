...
(function () {
  ...
  function start() {
    ...
    /* 用 config 里的图层名（如「导航」「卫星」）写入按钮 */
    var vName = (CFG.map && CFG.map.vectorLayerName) || '导航';
    var sName = (CFG.map && CFG.map.satelliteLayerName) || '卫星';
    var layerBtns = document.querySelectorAll('#layerCtrl .layer-btn');
    if (layerBtns[0]) layerBtns[0].textContent = vName;
    if (layerBtns[1]) layerBtns[1].textContent = sName;
    ...
    list.forEach(function (lm, i) {
      ...
      /* 导航 marker 顶部放实景照片（navPhoto），无图时退化为圆号码 */
      var photoUrl = lm.navPhoto || '';
      var html =
        '<div class="rm-pin" data-lm="' + lm.id + '">' +
          (photoUrl
            ? '<span class="rm-photo"><img src="' + photoUrl + '" alt="" loading="lazy" draggable="false"></span>'
            : '') +
      ...
      /* iconSize / iconAnchor 必须和 CSS 中 .rm-pin 实际尺寸一致，
         ...
         布局：照片 48 + 箭头 7 + 名称标签 24 ≈ 高 84，锚点在箭头尖(y=55)。 */
      var icon = L.divIcon({
        className: 'real-marker' + small,
        html: html,
        iconSize: [100, photoUrl ? 86 : 80],
        iconAnchor: [50, photoUrl ? 56 : 46]
      });
    ...
    function safeAction(fn) {
      try { fn(); } catch (e) { /* 忽略单次按钮异常 */ }
    }
    if (btnIn)  btnIn.addEventListener('click', function () { safeAction(function () { map.zoomIn(); }); });
    if (btnOut) btnOut.addEventListener('click', function () { safeAction(function () { map.zoomOut(); }); });
    if (btnRst) btnRst.addEventListener('click', function () { safeAction(function () { RealMap.fitAll(); }); });
    if (btnLoc) btnLoc.addEventListener('click', function () {
      if (!navigator.geolocation) {
        window.MapApp && window.MapApp.toast && window.MapApp.toast('该设备不支持定位');
        return;
      }
      btnLoc.classList.add('is-busy');
      navigator.geolocation.getCurrentPosition(function (p) {
        btnLoc.classList.remove('is-busy');
        map.flyTo([p.coords.latitude, p.coords.longitude], 16, { duration: 0.6 });
      }, function () {
        btnLoc.classList.remove('is-busy');
        window.MapApp && window.MapApp.toast && window.MapApp.toast('定位失败，请允许位置权限');
